...
Route::post('/password/otpsent', 'HomeController@ForgotPassword_verify')->name('user.password.varify');
Route::post('/users/login_with_password', 'HomeController@login_with_password')->name('login_with_password.submit');
Route::post('/password/submit', 'HomeController@PasswordSubmit')->name('password.submit');
Route::get('/users/forgot-password', 'HomeController@forgotPassword')->name('user.forgot.password');
Route::post('/users/otp/submit', 'HomeController@userOtpSubmit')->name('user.otp.submit');
Route::get('/users/forgotPassword', 'HomeController@NewPassword')->name('user.new.password');
Route::post('/users/newpassword', 'HomeController@userNewpassword')->name('user.newpassword');
...
Route::post('/contacts/store', 'ContactController@store')->name('contacts.stores');
Route::post('/questions/store', 'QuestionController@store')->name('questions.stores');
...
Route::get('/search?occasion={occasion_slug}', 'HomeController@search')->name('products.occasion');
Route::get('/search?fabric={fabric_slug}', 'HomeController@search')->name('products.fabric');
...
Route::get('/search?color={color_slug}', 'HomeController@search')->name('products.color');
Route::get('/search?tag={tags}', 'HomeController@search')->name('products.tag');
...
Route::get('/products', 'HomeController@search')->name('products');
Route::get('/product', 'HomeController@ajax_lode_more_product')->name('more.product');
Route::get('/search?type={diamond_type}', 'HomeController@ajax_lode_more_product')->name('products.type');
...
Route::get('/checkout/remove_coupon_code', 'CheckoutController@remove_coupon_code')->name('checkout.remove_coupon_code');
...
Route::get('/Shipping-policy', 'HomeController@exchangepolicy')->name('shippingpolicy');
...
Route::get('/buyback-policy', 'HomeController@byubackpolicy')->name('byubackpolicy');
...
Route::group(['middleware' => ['user', 'verified']], function(){
	...
	Route::post('/upload_doc', 'HomeController@upload_doc')->name('seller.upload_doc');
	...
	Route::get('/purchase_history/destroy/{id}', 'PurchaseHistoryController@destroy')->name('purchase_history.destroy');
        Route::get('/purchase_history_cancel_order/', 'PurchaseHistoryController@cancel_order_index')->name('purchase_history.cancel_purchase_history');
	
    Route::resource('wishlists','WishlistController');
	Route::post('/wishlists/remove', 'WishlistController@remove')->name('wishlists.remove');
	Route::post('/wishlists/new', 'WishlistController@updateWishlist')->name('wishlish.newwishlist');
	Route::get('/wallet', 'WalletController@index')->name('wallet.index');
	Route::post('/recharge', 'WalletController@recharge')->name('wallet.recharge');
	Route::post('/wishlists/rename/store', 'WishlistController@RenameStore')->name('wishlist.rename.store');
	Route::post('whishlist/share', 'WishlistController@wishlist_share')->name('whishlist.share');
	Route::get('wishlist/{slug}', 'WishlistController@WhishlistDetail')->name('whishlist.details');
	Route::post('/wishlists/store', 'WishlistController@NewWishlist')->name('wishlists.new');
	Route::post('/wishlists/rename', 'WishlistController@Wishlist_rename')->name('wishlists.rename');
	Route::post('/wishlists/rename/store', 'WishlistController@RenameStore')->name('wishlist.rename.store');
	Route::post('/wishlists/data', 'WishlistController@wishlistData')->name('wishlists.data');
    ...
    Route::get('/order/canceled/{id}', 'OrderController@canceled_order')->name('order.canceled');
	Route::get('order/{id}', 'OrderController@ViewOrder')->name('order.view');

	Route::get('user/profile', 'HomeController@profileEdit')->name('user.profile.edit');
	Route::get('/order/canceled/{id}', 'OrderController@canceled_order')->name('order.canceled');
    Route::get('/order-history', 'OrderController@order_history')->name('order.history');
    Route::get('/orderview/{id}', 'OrderController@OrderView')->name('order.view');
	Route::get('/gift-balance', 'HomeController@gift_balnce')->name('gift.balance');
});
...
Route::group(['middleware' => ['auth']], function(){
    ...
    Route::get('/orders_seller_canceled_order', 'OrderController@seller_canceled_order')->name('orders.seller_canceled_order');
	...
	Route::get('user/addresses/destroy/{id}', 'AddressController@destroy')->name('addresses.destroy');
	Route::get('user/addresses/set_default/{id}', 'AddressController@set_default')->name('addresses.set_default');
	Route::post('user/addresses/shipping', 'AddressController@shipping_address')->name('address.shipping');
	Route::get('user/addresses/shipping/{id}', 'AddressController@shipping_address_edit')->name('address.shipping.edit');
	Route::get('user/addresses/billing/{id}', 'AddressController@billing_address_edit')->name('address.billing.edit');

	Route::resource('/reviews', 'ReviewController');

	Route::post('pincode', 'HomeController@pincode')->name('pincode');
...
Route::post('nimbbl/payment/pay-success', 'NimbblController@payment')->name('payment.nimbbl');
...
Route::get('aboutus', 'HomeController@about')->name('abouts');
Route::get('contactus', 'HomeController@contact')->name('contacts');


Route::post('quick-view', 'HomeController@quickview')->name('quick.view');
