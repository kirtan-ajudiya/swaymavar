					...
					<td>
						@if (Auth::user()->user_type == 'seller')
							@if(Auth::user()->shop->logo != null)
								<img loading="lazy"  src="{{ asset('frontend/img/header-logo.png') }}" height="40" style="display:inline-block;">
							@else
								<img loading="lazy"  src="{{ asset('frontend/img/header-logo.png') }}" height="40" style="display:inline-block;">
							@endif
						@else
	                ...
	                @foreach ($order->orderDetails->where('seller_id', $user_id) as $key => $orderDetail)
		                @if ($orderDetail->product)
							<tr class="">
								...
								<td class="gry-color currency">{{ single_price_without_currancy($orderDetail->price/$orderDetail->quantity) }}</td>
								<td class="gry-color currency">{{ single_price_without_currancy($orderDetail->tax/$orderDetail->quantity) }}</td>
			                    <td class="text-right currency">{{ single_price_without_currancy($orderDetail->price+$orderDetail->tax) }}</td>
							</tr>
		                @endif
					@endforeach
	            </tbody>
	        ...
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			            ...
			            <td class="currency">{{ single_price_without_currancy($order->orderDetails->where('seller_id', $user_id)->sum('price')) }}</td>
			        </tr>
			        <tr>
			            <th class="gry-color text-left">Shipping Cost</th>
			            <td class="currency">{{ single_price_without_currancy($order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost')) }}</td>
			        </tr>
			        <tr class="border-bottom">
			            <th class="gry-color text-left">Total Tax</th>
			            <td class="currency">{{ single_price_without_currancy($order->orderDetails->where('seller_id', $user_id)->sum('tax')) }}</td>
			        </tr>
			        <tr>
			            <th class="text-left strong">Grand Total</th>
			            <td class="currency">{{ single_price_without_currancy($order->orderDetails->where('seller_id', $user_id)->sum('price') + $order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost') + $order->orderDetails->where('seller_id', $user_id)->sum('tax')) }}</td>
			        </tr>
		        </tbody>
		    </table>
	    </div>
