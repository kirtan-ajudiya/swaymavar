...
@section('content')
    ...
    <div class="panel">
        <div class="panel-heading bord-btm clearfix pad-all h-100">
            <h3 class="panel-title pull-left pad-no">{{__('Affiliate Users')}}</h3> <a href="{{ route('affiliate.get_user_for_affiliate') }}" class="btn btn-primary pull-right" type="button">
                                        {{__('Set Default Affiliat')}}
                                    </a>
        </div>
        <?php 
        $pendingAmt=0;
                    ...
                    @if($affiliate_user->user != null)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @else {{ single_price(0) }} @endif
                            </td>
                            <td>
                                @if($affiliate_user->isdefault == 1)
                            <i class="fa fa-circle" style="font-size: 20px;margin-left: 30%;color: green;"
                               ></i>
                                @else
                            <i class="fa fa-circle" style="font-size: 20px;margin-left: 30%;color: red;"
                               ></i>
                               @endif
                            </td>
                            <td>
                                <div class="btn-group dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right">
                                        ...
                                        <li><a href="{{route('affiliate.subusers',$affiliate_user->user_id)}}">{{__('Under Users')}}</a></li>

                                        <li><a href="{{route('affiliate.tree_view',$affiliate_user->user_id)}}">{{__('Tree View')}}</a></li>

                                        <li><a href="{{route('affiliate.get_commision_logs',$affiliate_user->user_id)}}">{{__('Comminsion History')}}</a></li>
