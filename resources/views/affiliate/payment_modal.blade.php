    ...
    <div class="modal-body">
        <div>
            <table class="table table-responsive">
                <tbody>
                    <tr>
                        @if(($affiliate_user->balance - $pendingAmt) >= 0)
                            <td>{{ __('Due Amount') }}</td>
                            <td><strong>{{ single_price($affiliate_user->balance - $pendingAmt) }}</strong></td>
                        @endif
                    </tr>
                    <tr>
        ...
        @if (($affiliate_user->balance - $pendingAmt) > 0)
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <input type="number" min="0" step="0.01" name="amount" id="amount" value="{{ $affiliate_user->balance - $pendingAmt }}" class="form-control" required>
                </div>
            </div>
