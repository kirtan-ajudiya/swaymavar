...
<div class="more hide">
    <div class="product-choose">
        ...
        {{-- <div class="form-group" id="subcategory">
           ...
           <div class="col-lg-9">
              <select class="form-control subcategory_id" name="subcategory_ids[]" onchange="get_products_by_subsubcategory(this)">
...
<script type="text/javascript">
    ...
    //     var category_id = $(el).val();
    //     $(el).closest('.product-choose').find('.product_id').html(null);
    //     $.post('{{ route('products.get_products_by_subsubcategory') }}',{_token:'{{ csrf_token() }}', subcategory_id:subcategory_id,category_id:category_id }, function(data){
    //         for (var i = 0; i < data.length; i++) {
    //             $(el).closest('.product-choose').find('.product_id').append($('<option>', {
    //                 value: data[i].id,
    //                 text: data[i].name
