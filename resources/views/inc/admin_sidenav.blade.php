                        ...
                        @if(Auth::user()->user_type == 'admin' || in_array('1', json_decode(Auth::user()->staff->role->permissions)))
                            <li>
                                ...
                                {{-- <li class="{{ areActiveRoutes(['brands.index', 'brands.create', 'brands.edit'])}}">
                                    ...
                                    <li class="{{ areActiveRoutes(['metaltypes.index', 'metaltypes.create', 'metaltypes.edit'])}}">
                                        <a class="nav-link" href="{{route('metaltypes.index')}}">{{__('Metal Type')}}</a>
                                    </li>
                                   
                                    <li class="{{ areActiveRoutes(['metalcolors.index', 'metalcolors.create', 'metalcolors.edit'])}}">
                                        <a class="nav-link" href="{{route('metalcolors.index')}}">{{__('Metal Color')}}</a>
                                    </li>
                                    <li class="{{ areActiveRoutes(['metalprices.index', 'metalprices.create', 'metalprices.edit'])}}">
                                        <a class="nav-link" href="{{route('metalprices.index')}}">{{__('Metal caret')}}</a>
                                    </li>
                                    <li class="{{ areActiveRoutes(['jewellery_types.index', 'jewellery_types.create', 'jewellery_types.edit'])}}">
                                        <a class="nav-link" href="{{route('jewellery_types.index')}}">{{__('Jewellery Type')}}</a>
                                    </li>
                                    <!-- <li class="{{ areActiveRoutes(['fabric_categories.index', 'fabric_categories.create', 'fabric_categories.edit'])}}">
                                        <a class="nav-link" href="{{route('fabric_categories.index')}}">{{__('Fabric')}}</a>
                                    </li>
                                    <li class="{{ areActiveRoutes(['occasion_categories.index', 'occasion_categories.create', 'occasion_categories.edit'])}}">
                                        <a class="nav-link" href="{{route('occasion_categories.index')}}">{{__('Occasion')}}</a>
                                    </li> -->
...
{{-- 
                        ...
                        @if(Auth::user()->user_type == 'admin' || in_array('3', json_decode(Auth::user()->staff->role->permissions)))
                            @php
                                $orders = DB::table('orders')
                                            ...
                                            ->where('orders.viewed', 0)->where('order_details.delivery_status','!=', 'canceled')
                                            ->select('orders.id')
                                            ->distinct()
                                            ->count();
                            @endphp
                        ...
                        <li class="{{ areActiveRoutes(['orders.orders_canceled.admin', 'orders.show'])}}">
                            <a class="nav-link" href="{{ route('orders.orders_canceled.admin') }}">
                                <i class="fa fa-remove"></i>
                                <span class="menu-title">{{__('Canceled orders')}}</span>
                            </a>
                        </li>
                        @endif
...
<!-- 
                       ...
                       {{-- @if(Auth::user()->user_type == 'admin' || in_array('16', json_decode(Auth::user()->staff->role->permissions)))
                        <li class="{{ areActiveRoutes(['create_notification'])}}">
                            <a class="nav-link" href="{{ route('create_notification') }}">
                        ...
                        @if(Auth::user()->user_type == 'admin' || in_array('16', json_decode(Auth::user()->staff->role->permissions)))
                        @php
                            $contact = \App\Contact::where('view', 1)->count();
                        @endphp
                        <li class="{{ areActiveRoutes(['contacts.index'])}}">
                            <a class="nav-link" href="{{ route('contacts.index') }}">
                                <i class="fa fa-id-card"></i>
                                <span class="menu-title">{{__('Contact')}} @if($contact > 0)<span class="pull-right badge badge-info">{{ $contact }}</span>@endif</span>
                            </a>
                        </li>
                        @endif
                        @if(Auth::user()->user_type == 'admin' || in_array('16', json_decode(Auth::user()->staff->role->permissions)))
                        @php
                            $question = \App\Question::where('view', 1)->count();
                        @endphp
                        <!-- <li class="{{ areActiveRoutes(['questions.index'])}}">
                            <a class="nav-link" href="{{ route('questions.index') }}">
                                <i class="fa fa-question"></i>
                                <span class="menu-title">{{__('Questions')}}@if($question > 0)<span class="pull-right badge badge-info">{{ $question }}</span>@endif</span>
                            </a>
                        </li> -->
                        @endif
                                 ...
                                 <li>
                                    ...
                                    <ul class="collapse">
                                        ...
                                        <li class="{{ areActiveRoutes(['sellerpolicy.index'])}}">
                                            <a class="nav-link" href="{{route('sellerpolicy.index', 'buyback_policy')}}">{{__('Buyback Policy')}}</a>
                                        </li>
                        ...
                        @if (\App\Addon::where('unique_identifier', 'affiliate_system')->first() != null)
                            <li>
                                <a href="#">
                                    ...
                                    <span class="menu-title">{{__('MLM System')}}</span>
                                    <i class="arrow"></i>
                                </a>
...
<!--                                    <li class="{{ areActiveRoutes(['affiliate.configs'])}}">
                                    ...
                                    <li class="{{ areActiveRoutes(['affiliate.users', 'affiliate_users.show_verification_request', 'affiliate_user.payment_history'])}}">
                                        <a class="nav-link" href="{{route('affiliate.users')}}">{{__('MLM Users')}}</a>
                                    </li>
                                </ul>
                            </li>
                        @endif
                        ...
                        {{-- @if(Auth::user()->user_type == 'admin' || in_array('13', json_decode(Auth::user()->staff->role->permissions)))
                            @php
                                $support_ticket = DB::table('tickets')
                                            ...
                                            ->count();
                            @endphp
                            <li class="{{ areActiveRoutes(['support_ticket.admin_show'])}}">
                            <a href="#" >
                                <i class="fa fa-support"></i>
                                <span class="menu-title">{{__('Suppot Ticket')}} @if($support_ticket > 0)<span class="badge badge-info" style="margin-left:35%;">{{ $support_ticket }}</span>@endif</span>
                                <i class="arrow"></i>
                            </a>
                            ...
                            <ul class="collapse">
                                <li class="{{ areActiveRoutes(['support_ticket.admin_index'])}}">
                                    <a class="nav-link" href="{{ route('support_ticket.admin_index') }}">{{__('Suppot Ticket Pending')}}</a>
                                </li>
                                <li class="{{ areActiveRoutes(['support_ticket.admin_solved'])}}">
                                    <a class="nav-link" href="{{ route('support_ticket.admin_solved') }}">{{__('Suppot Ticket Solved')}}</a>
                                </li>
                            </ul>
                        </li>
                        @endif --}}
