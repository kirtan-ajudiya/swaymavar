                ...
                <div class="box-inline pad-rgt pull-left">
                    <div class="select" style="min-width: 300px;">
                        <select class="form-control demo-select2" name="delivery_status" id="delivery_status" onchange="sort_orders()">
                            ...
                            <option value="pending"   @isset($delivery_status) @if($delivery_status == 'pending') selected @endif @endisset>{{__('Packing')}}</option>
                            <option value="on_review"   @isset($delivery_status) @if($delivery_status == 'on_review') selected @endif @endisset>{{__('Dispatching')}}</option>
                ...
                @foreach ($orders as $key => $order_id)
                    ...
                    @if($order != null)
                        <tr>
                            ...
                            @endif
                            <!-- <td>   @if($order->isshiprocket == 0)
                                   @if($order->orderDetails->first()->delivery_status == "delivered" || $order->orderDetails->first()->delivery_status == "canceled")  
                                   <a href="" class="btn btn-primary disabled" >{{__('Send Shiprocket')}}</a>
                                   @else
                                   <a href="{{route('orders.shiprocket.admin',encrypt($order->id))}}" class="btn btn-primary " >{{__('Send Shiprocket')}}</a>
                                   @endif
                                   @else
                                   @if($order->orderDetails->first()->delivery_status == "delivered")  
                                   <a href="" class="btn btn-danger disabled" >{{__('Tracking')}}</a>
                                   @else
                                   <a href="" class="btn btn-danger " >{{__('Tracking  Order')}}</a>
                                   @endif
                                   @endif
                            </td> -->
                            <td>
