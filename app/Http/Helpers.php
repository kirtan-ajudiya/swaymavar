...
if (! function_exists('areActiveRoutes')) {
    function areActiveRoutes(Array $routes, $output = "current-cat")
    {
...
if (! function_exists('areActiveRoutesHome')) {
    function areActiveRoutesHome(Array $routes, $output = "w--current")
    {
        foreach ($routes as $route) {
            if (Route::currentRouteName() == $route) return $output;
        }
...
if (! function_exists('areActiveCategory')) {
    function areActiveCategory(Array $categorys, $output = "current-cat")
    {
        foreach ($categorys as $category) {
            if (Category::where('name' == $category)) return $output;
        }
...
if (! function_exists('format_price')) {
    function format_price($price)
    {
       ...
       return number_format(convert_price($price), BusinessSetting::where('type', 'no_of_decimals')->first()->value).currency_symbol();
    }
}
...
//formats price to home default price with convertion
if (! function_exists('single_price_without_currancy')) {
    function single_price_without_currancy($price)
    {
        return ('Rs ' . convert_price($price));
    }
}
...
if(! function_exists('renderStarRating')){
    function renderStarRating($rating,$maxRating=5) {
        $fullStar = "<div class = 'fa is-filled'></div>";
        $halfStar = "<i class = 'fa fa-star half'></i>";
        $emptyStar = "<div class = 'fa is-blank'></div>";
...
if (! function_exists('BaseTotalPrice')) {
    function BaseTotalPrice($id)
    { 
       $price = Product::findOrfail($id);
        $price = $price->sub_total;
        return $price;
    }
...
if (! function_exists('TotalTax')) {
    function TotalTax($id)
    { 
       $product = Product::findOrfail($id);
       $tax = "";
        if($product->tax_type == 'percent'){
            if($product->tax != '0.00'){
                $tax = (BaseTotalPrice($id) * $product->tax) / 100;
            }
        }else{
            $tax = $product->tax; 
        }
...
if (! function_exists('TotalDiscount')) {
    function TotalDiscount($id)
    { 
       $product = Product::findOrfail($id);
       $discount = "";
        if($product->discount_type == 'percent'){
            if($product->discount != '0.00'){
                $discount = (BaseTotalPrice($id) * $product->discount) / 100;
            }
        }else{
...
if (! function_exists('TotalPrice')) {
    ...
    { 
        $product = Product::findOrfail($id);
        $price = "";
        $price = BaseTotalPrice($id) + TotalTax($id) - TotalDiscount($id);
        return $price;
    }
}


if (! function_exists('FrontTotalPrice')) {
    function FrontTotalPrice($id)
    { 
        $product = Product::findOrfail($id);
        $price = "";
        $price = $product->sub_total + TotalTax($product->id);
        return $price;
    }
}

if (! function_exists('FrontdiscountTotalPrice')) {
    function FrontdiscountTotalPrice($id)
    { 
        $product = Product::findOrfail($id);
        $price = "";
        $price = $product->unit_price;
        if ($product->discount_type == 'percent') {
            $ftotal = (FrontTotalPrice($id)*$price->discount)/100;
            $price = $jwelleryprice -  $ftotal;
        } elseif ($product->discount_type == 'amount') {
            $price =  FrontTotalPrice($id) - $product->discount;
        }
...
if (! function_exists('TotalProductWeight')) {
    function TotalProductWeight($id)
    { 
        $product = Product::findOrfail($id);
        $weight = "";
        $weight = $product->metal_weight + $product->diamond_weight  + $product->solited_diamond_weight  + $product->choki_diamond_weight ;
        return $weight;
    }
}

if (! function_exists('ProductFinalValue')) {
    function ProductFinalValue($id)
    { 
        $product = Product::findOrfail($id);
        $price = "";
        $price = ($product->metal_weight * $product->metal_rate) + ($product->diamond_weight * $product->diamond_price)  + 
                ($product->solited_diamond_weight * $product->solited_diamond_price) + ($product->choki_diamond_weight * $product->choki_diamond_price) ;
        return $price;
    }
}

if (! function_exists('totalTaxProduct')) {
    function totalTaxProduct($id)
    { 
        $product = Product::findOrfail($id);
        $fianlprice = "";
        $type = $product->tax_type;
		$price = $product->sub_total;
		$tax =  $product->tax;
		if($type == 'percent'){
			$fianlprice = (($price*$tax)/100) ;
		}else{
			$fianlprice = $product->tax;
		}
        return $fianlprice;
