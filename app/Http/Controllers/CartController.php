...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $data = array();
        $data['id'] = $product->id;
        $data['maincategory'] = $product->category_id;
        $str = '';
        $tax = 0;
        ...
        if($str != null && $product->variant_product){
            ...
            $originprice = $product_stock->price;
            ...
            $originprice = $product->unit_price;
        }
        ...
        if (!$inFlashDeal) {
            if(Auth::check() && Auth::user()->pro_member == 1 && isset(Auth::user()->package_name) && Auth::user()->package_name->approved == 1){
                if($product->discount_type == 'percent'){
                    $price -= ($price*$product->paid_discount)/100;
                }
                elseif($product->discount_type == 'amount'){
                    $price -= $product->paid_discount;
                }
             }else{
                    if($product->discount_type == 'percent'){
